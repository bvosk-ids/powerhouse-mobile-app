...
function Migrator(db){
    ...
    this.execute = function(){
        ...
        // Always create migration table
        db.transaction(function(t1) {
            t1.executeSql("create table " + MIGRATOR_TABLE + "(version integer)", [], function(t2){
                t2.executeSql("insert into " + MIGRATOR_TABLE + " values(0)", [], function(){
                    debug(Migrator.DEBUG_HIGH, "New migration database created...");
                    migrateStartingWith(0);
                }, function(t3, err){
                    debug(Migrator.DEBUG_HIGH, 'Error inserting into migrator table');
                });
            }, function(t4, err){
                debug(Migrator.DEBUG_HIGH, 'Error creating migrator table');
            });
        });

        db.transaction(function(t1){    
            t1.executeSql("select version from " + MIGRATOR_TABLE, [], function(t2, res){        
                var rows = res.rows;
                ...
                if(err.message.match(/no such table/i)){
                    t3.executeSql("create table " + MIGRATOR_TABLE + "(version integer)", [], function(t4){
                        t4.executeSql("insert into " + MIGRATOR_TABLE + " values(0)", [], function(){
                            debug(Migrator.DEBUG_HIGH, "New migration database created...");
                            migrateStartingWith(0);
                        }, function(t5, err){
